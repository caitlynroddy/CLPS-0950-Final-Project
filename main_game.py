...
def main():
    ...
    def show_board(play_board): #Drawing the game board graphic.#
        for col in range(number_of_columns):
            for r in range(number_of_rows):
                pygame.draw.rect(screen, white, (col*square_pixels, r*square_pixels+square_pixels, square_pixels, square_pixels)) #The screen is the surface. The white is the predefined white (255,255, 255). We multiply rows times the size of the square (and add the size in order to create the top row where the piece will be), the columns times the size of the square, establish the height and width, to create the rectangle. #
                pygame.draw.circle(screen, black, (int(col*square_pixels + square_pixels/2), int(r*square_pixels+square_pixels+square_pixels/2)), rad) #Using the pygame command for circles that represent the pieces.#
        ...
        for col in range(number_of_columns):
            for r in range(number_of_rows):
                if play_board[r][col] == 1: #Player 1 piece color.#
                    ...
                    pygame.draw.circle(screen, lime, (int(col*square_pixels + square_pixels/2), screen_length - int(r*square_pixels + square_pixels/2)), rad)
        ...
        for event in pygame.event.get(): #Events are actions, such as clicking and mouse motions, that are part of the game.#
            ...
            if event.type == pygame.MOUSEMOTION: #Makes the players able to see their piece before dropping it into the slot. For visual effect.#
                ...
                if player_turn == 0: #Player 1's turn.#
                    ...
                    pygame.draw.circle(screen, lime, (posx, int(square_pixels/2)), rad)
                pygame.display.update()
            ...
            #Player 2 Turn#
                    ...
                    if win_check(play_board, 2): #Function to check if player 2 wins.#
                            label = myfont.render("Player Two, you win!",1, lime) #Displays winning message to player 2 if they win.#
